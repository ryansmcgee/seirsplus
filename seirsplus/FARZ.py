...
def choose_node(i,c, G, C, alpha, beta, gamma, epsilon):
    ...
    if random.random()<epsilon or len(trim_ids)<=0:
        ...
        tmp = random_choice(list(range(len(p))), p ) #, size=1, replace = False)
...
def select_node(G, method = 'uniform'):
    if method=='uniform':
        ...
        return  random_choice(list(range(len(p))), p ) #, size=1, replace = False)[0]

def assign(i, C, e=1, r=1, q = 0.5):
    p = [e +len(c) for c in C.groups]
    id = random_choice(list(range(C.k)),p )
    C.add(id, i)
...
def print_setting(n,m,k,alpha,beta,gamma, phi,r,q,epsilon,weighted,directed):
    print('n:',n,'m:', m ,'k:', k,'alpha:', alpha,'beta:', beta,'gamma:', gamma, end=' ')
    if phi!=default_FARZ_setting['phi']: print('phi:', phi, end=' ')
    if r!=default_FARZ_setting['r']: print('r:', r, end=' ')
    if q!=default_FARZ_setting['q']: print('pr:', q, end=' ')
...
def generate( vari =None, arange =None, repeat = 1, path ='.', net_name = 'network',format ='gml', farz_params= default_FARZ_setting.copy()):
    ...
    if vari is None:
        for r in range(repeat):
            ...
            node_communities = {node: [c[0] for c in comm_tup] for node, comm_tup in list(C.memberships.items())}
