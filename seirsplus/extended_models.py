...
class ExtSEIRSNetworkModel():
    ...
    Params:
            ...
            d_sym           Probability of positive test for infectious symptomatic individuals
            d_asym          Probability of positive test for infectious asymptomatic individuals
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def update_parameters(self):
        ...
        self.d_sym          = numpy.array(self.parameters['d_sym']).reshape((self.numNodes, 1))         if isinstance(self.parameters['d_sym'], (list, numpy.ndarray)) else numpy.full(fill_value=self.parameters['d_sym'], shape=(self.numNodes,1))
        self.d_asym         = numpy.array(self.parameters['d_asym']).reshape((self.numNodes, 1))        if isinstance(self.parameters['d_asym'], (list, numpy.ndarray)) else numpy.full(fill_value=self.parameters['d_asym'], shape=(self.numNodes,1))
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def calc_propensities(self):
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            propensities_StoQS       = (self.theta_S + self.phi_S*numContacts_Q)*self.d_S * (self.X==self.S)

            propensities_EtoQE       = (self.theta_E + self.phi_E*numContacts_Q)*self.d_E * (self.X==self.E)

            propensities_IPREtoQPRE  = (self.theta_pre + self.phi_pre*numContacts_Q)*self.d_pre * (self.X==self.I_pre)

            propensities_ISYMtoQSYM  = (self.theta_sym + self.phi_sym*numContacts_Q)*self.d_sym * (self.X==self.I_sym)

            propensities_IASYMtoQASYM = (self.theta_asym + self.phi_asym*numContacts_Q)*self.d_asym * (self.X==self.I_asym)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_iteration(self):
        ...
        if(propensities.sum() > 0):
            ...
            # print(str(self.t) + " " + str(transitionNode) + " " + transitionType)
            ...
            if(transitionType == 'StoE' or transitionType == 'QStoQE'):
                ...
                self.infectionsLog.append({ 't':                            self.t,
                                            ...
                                            'local_contact_node_states':    self.X[[int(a) for a in transitionNode_GNbrs]].flatten(),
                                            'isolation_contact_nodes':      transitionNode_GQNbrs,
                                            'isolation_contact_node_states':self.X[[int(a) for a in transitionNode_GNbrs]].flatten() })
