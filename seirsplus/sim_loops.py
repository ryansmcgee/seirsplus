...
def escalation_testing_sim(model, T, N, screening_prop = 1/30, escalated_prop=1/7, temporal_falseneg_rates=get_temporal_false_rates(), testing_interval =1):

        model.tmax  = T
        running = True
        escalation_mode = False
        cur_interval = screening_prop
        timeOfLastTestingInterval=0
        first_positive_day=0
        while running:
            running = model.run_iteration()
            if(int(model.t)%testing_interval == 0 and int(model.t)!=int(timeOfLastTestingInterval)):
                timeOfLastTestingInterval = int(model.t)
                nodeStates = model.X.flatten()
                testingPool = numpy.argwhere(((nodeStates != model.R)
                                             & (nodeStates != model.Q_R)
                                             & (nodeStates != model.H)
                                             & (nodeStates != model.F)
                                             & (nodeStates !=model.Q_sym)
                                             & (nodeStates !=model.Q_asym)
                                             & (nodeStates !=model.Q_pre))
                                            ).flatten()

                numRandomTests = int(N * cur_interval)

                testingPool_degrees       = model.degree.flatten()[testingPool]
                if(len(testingPool) > 0):
                    randomSelection = testingPool[numpy.random.choice(len(testingPool), numRandomTests, replace=False)]

                #----------------------------------------
                # Perform the tests on the selected individuals:
                #----------------------------------------
                selectedToTest = randomSelection
                # each day, test some people
                for i, testNode in enumerate(selectedToTest):
                        ...
                        pass
                         ...
                         if(numpy.random.rand() < (1-falseneg_prob)):
                            ...
                            if not escalation_mode:
                                first_positive_day = model.t
                                print(f'Found first positive on day: {first_positive_day}')
                                escalation_mode = True
                                cur_interval = escalated_prop

                            # Update the node's state to the appropriate detcted case state:
                            model.set_positive(testNode, True)
                            model.set_isolation(testNode, True)

        return(first_positive_day)
